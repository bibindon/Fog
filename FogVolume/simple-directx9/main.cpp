...
LPD3DXMESH          g_meshCube = NULL;   // Opaque: cube.x
LPD3DXMESH          g_meshSphere = NULL;   // Fog:    sphere.blend.x
LPDIRECT3DTEXTURE9  g_texGrass = NULL;   // grass.png
...
static DWORD GetSubsetCount(LPD3DXMESH m) { DWORD n = 0; if (!m) return 0; m->GetAttributeTable(NULL, &n); return n ? n : 1; }
...
{
    HRESULT hr;
    g_pD3D = Direct3DCreate9(D3D_SDK_VERSION); assert(g_pD3D);

    D3DPRESENT_PARAMETERS pp = {};
    pp.Windowed = TRUE; pp.SwapEffect = D3DSWAPEFFECT_DISCARD; pp.BackBufferFormat = D3DFMT_UNKNOWN;
    pp.EnableAutoDepthStencil = TRUE; pp.AutoDepthStencilFormat = D3DFMT_D24S8; pp.hDeviceWindow = hWnd;

    hr = g_pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWnd, D3DCREATE_HARDWARE_VERTEXPROCESSING, &pp, &g_pd3d);
    if (FAILED(hr)) { hr = g_pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWnd, D3DCREATE_SOFTWARE_VERTEXPROCESSING, &pp, &g_pd3d); assert(SUCCEEDED(hr)); }

    // メッシュ
    hr = D3DXLoadMeshFromX(_T("cube.x"), D3DXMESH_MANAGED, g_pd3d, NULL, NULL, NULL, NULL, &g_meshCube);   assert(SUCCEEDED(hr));
    hr = D3DXLoadMeshFromX(_T("sphere.blend.x"), D3DXMESH_MANAGED, g_pd3d, NULL, NULL, NULL, NULL, &g_meshSphere); assert(SUCCEEDED(hr));

    // テクスチャ
    hr = D3DXCreateTextureFromFile(g_pd3d, _T("grass.png"), &g_texGrass); assert(SUCCEEDED(hr));

    // FX
    hr = D3DXCreateEffectFromFile(g_pd3d, _T("simple.fx"), NULL, NULL, D3DXSHADER_DEBUG, NULL, &g_fx, NULL); assert(SUCCEEDED(hr));

    // RTs
    CreateRenderTargets(WINDOW_W, WINDOW_H);
...
void Render()
{
    ...
    D3DXVECTOR3 eye(8.0f * sinf(t), 5.0f, -8.0f * cosf(t)), at(0, 0, 0), up(0, 1, 0);
    D3DXMATRIX mV, mP; D3DXMatrixLookAtLH(&mV, &eye, &at, &up);
    D3DXMatrixPerspectiveFovLH(&mP, D3DXToRadian(45.0f), (float)WINDOW_W / WINDOW_H, 0.5f, 1000.0f);
